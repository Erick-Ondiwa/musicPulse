...
export const musicApi = {
  ...
  ingestPopular: async ({ regionCode = "KE", maxResults = 25 } = {}) =>
    (
...
//   (error) => {
//     const message =
//       error.response?.data?.detail || error.response?.data?.message || error.message || "The request failed.";
//     return Promise.reject(new Error(message));
//   },
// );
...
// export const musicApi = {
//   latestSongs: async ({ limit = 10, hours } = {}) =>
//     (await api.get("/songs/latest", { params: { limit, ...(hours ? { hours } : {}) } })).data,
//   trendingSongs: async ({ limit = 10, lookbackHours = 24 } = {}) =>
//     (await api.get("/songs/trending", { params: { limit, lookback_hours: lookbackHours } })).data,
//   mostViewedSongs: async ({ limit = 10, publishedWithinHours } = {}) =>
//     (await api.get("/songs/most-viewed", { params: { limit, ...(publishedWithinHours ? { published_within_hours: publishedWithinHours } : {}) } })).data,
//   topArtists: async ({ limit = 10 } = {}) =>
//     (await api.get("/artists/top", { params: { limit } })).data,
//   askAssistant: async ({ question, conversationId }) =>
//     (await api.post("/assistant/ask", { question, conversation_id: conversationId || null })).data,
...
//   listKnowledge: async () => (await api.get("/knowledge/documents")).data,
//   syncKnowledge: async () => (await api.post("/knowledge/sync-videos")).data,
//   addKnowledge: async (payload) => (await api.post("/knowledge/documents", payload)).data,
//   ingestPopular: async ({ regionCode = "KE", maxResults = 25 } = {}) =>
//     (await api.post("/ingestion/popular", null, { params: { region_code: regionCode, max_results: maxResults } })).data,
//   ingestRecent: async ({ regionCode = "KE", hours = 24, maxResults = 25 } = {}) =>
//     (await api.post("/ingestion/recent", null, { params: { region_code: regionCode, hours, max_results: maxResults } })).data,
//   refreshStatistics: async () => (await api.post("/ingestion/refresh-statistics")).data,
// };
